...
Ce benchmark a déjà payé deux fois. Il a révélé qu'un garde-fou supprimait à tort les noms en format formulaire (« Nom: Dupont ») — corrigé, sans un seul faux positif de plus sur le jeu interne. Et il a chiffré le plafond de la NER française, ce qui a justifié la couche suivante.

**Le juge local franchit ce plafond.** Sur 40 lignes du même jeu (32 noms annotés), avec `qwen2.5:14b` en local :
...
Le rappel passe de 41 % à 94 % — 17 noms rattrapés sur 32 — et la précision augmente aussi. Le prix est une latence multipliée par 41 : la couche est donc **désactivée par défaut** et s'active par client (`deep_scan`), flux par flux. Détail, garde-fous et limites : [`docs/juge-local.md`](docs/juge-local.md).
...
**Détection en profondeur (5 couches).** Chaque couche dégrade proprement si indisponible — et son absence est **journalisée**, jamais silencieuse : sur un outil de détection, dégrader sans le dire laisserait croire qu'un texte est propre alors qu'il n'a pas été analysé. Un IBAN est validé par registre SWIFT *et* clé mod-97, une carte par Luhn, un NIR par sa clé de contrôle. Les noms passent par Presidio avec des garde-fous anti-faux-positifs (un verbe capitalisé en début de phrase n'est pas un prénom).
...
**Journal d'audit inviolable, cloisonné par client.** Chaque organisation a **sa propre chaîne de hachage** : elle se vérifie seule, s'exporte seule (`/audit/export`), et une altération chez un client n'invalide pas la preuve d'un autre.
...
| `/audit/export` | GET | clé SENTINEL | Export du journal **du client** (chaînage vérifiable) |
| `/policy` | GET | clé SENTINEL | Politique de détection du client |
| `/policy` | PUT | clé SENTINEL | Régler exceptions, seuils et actions |
...
- [x] Détection multilingue (FR/EN + ~100 langues via modèle multilingue, détection auto)
- [x] Chiffrement fort du journal (Fernet AES-128 + HMAC) et effacement RGPD par crypto-shredding (`/compliance/forget`)
- [x] [Politique de conservation documentée](docs/politique-retention.md) : ce qui est stocké, chiffré comment, purgé quand
- [x] Vault relu depuis la base : la restauration des vraies valeurs survit à un redémarrage et fonctionne en multi-workers ; TTL réellement purgé
...
- [x] **Compatibilité OpenAI réelle** : `tools`, contenu multimodal, `response_format`, `seed`, `top_p` relayés au lieu d'être abandonnés en silence ; les arguments d'appel d'outil sont assainis à l'aller et **restaurés au retour**, y compris en streaming
...
Test suite: **375 unit, integration and API tests, all green** in CI on Python 3.11 and 3.12.
...
| `/audit/export` | GET | SENTINEL key | Export the **client's own** log (verifiable linkage) |
| `/policy` | GET | SENTINEL key | Client's detection policy |
| `/policy` | PUT | SENTINEL key | Tune exceptions, thresholds and actions |
...
- [x] Multilingual detection (FR/EN + ~100 languages via a multilingual model, auto language detection)
- [x] Strong audit-log encryption (Fernet AES-128 + HMAC) and GDPR erasure via crypto-shredding (`/compliance/forget`)
- [x] [Documented retention policy](docs/politique-retention.md): what is stored, how it is encrypted, when it is purged
- [x] Vault re-read from the database: real-value restoration survives restarts and works across workers; TTL actually purged
...
- [x] **Real OpenAI compatibility**: `tools`, multimodal content, `response_format`, `seed`, `top_p` forwarded instead of silently dropped; tool-call arguments sanitised outbound and **restored inbound**, streaming included
